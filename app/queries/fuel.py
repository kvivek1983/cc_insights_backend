...
WHERE is_deleted = false AND company_id = $1
  AND fuel_expense_id != 12 AND total_amount < 1000000 AND volume_in_liter BETWEEN 1 AND 500 AND fuel_price BETWEEN 50 AND 150
  AND expense_date >= CURRENT_DATE - ($2::text || ' days')::interval AND expense_date <= CURRENT_DATE
...
WHERE is_deleted = false AND company_id = $1
  AND fuel_expense_id != 12 AND total_amount < 1000000 AND volume_in_liter BETWEEN 1 AND 500 AND fuel_price BETWEEN 50 AND 150
  AND expense_date >= DATE_TRUNC('month', CURRENT_DATE) - ($2::text || ' months')::interval
...
WHERE fe.is_deleted = false AND fe.company_id = $1
  AND fe.fuel_expense_id != 12 AND fe.total_amount < 1000000 AND fe.volume_in_liter BETWEEN 1 AND 500 AND fe.fuel_price BETWEEN 50 AND 150
